...
class Base extends CI_Controller
{
    ...
    public function violation($plate = NULL, $offence = 1){
        /*$data               = array('data'=>'from pi');
        $this->insert('tbl_resp',$data);
        if (!is_null($plate)){
            $data               = array('data'=>$plate);
            $this->insert('tbl_resp',$data);
            http_response_code(200);
            echo json_encode(array('status'=>true,'plate'=>str_replace("%20"," ",$plate)));
        }else{
            http_response_code(701);
            echo json_encode(array('status'=>true,'plate'=>str_replace("%20"," ",$plate)));
        }*/

        //pi inatumia hii function kutuma data
        // hii function inapokea plate number

        if (!is_null($plate) && !is_null($offence)){
            //hapa inacheck plate number kwenye database kama imekuwa registered
            $condition = array(
                "plate_number" => str_replace("%20"," ",$plate)
            );
            $car = $this->get('tbl_car',$condition,null);
            $condition          = array('user_id'=>$car->owner_id);
            $user               = $this->get('tbl_users',$condition,null);
            $control_number         = rand(111111,999999);
            $message                = "Your car with reg no : $car->plate_number has violated traffic rule. Please pay with control number : $control_number, TSH 50,000,00 /=";
            //once ikiwa registered itachukua phonenumber ya huyo mtu na kutuma sms
            if ($car){
                //detail za vilation zinakuwa stored hapa na kutumwa kwenye database
                $data           = array(
                    'offence_id'    => $offence,
                    'car_id'        => $car->car_id,
                    'sms'           => $message,
                    'control_number'        => $control_number,
                    'violation_time' => self::time()
                );
                $this->insert('tbl_violation',$data);
            }
            //haaaapa inatuma sms kwa user
    ...
    public function owner($data = null,$operation = 0){
        $pass = false;
        if ($operation == 1){
            $this->form_validation->set_rules('phone', 'owner phone', 'trim|required');
            $this->form_validation->set_rules('car', 'registration car', 'trim|required');
            if ($this->form_validation->run()) {
                ...
                if ($user){
                    ...
                    $regCar = $this->get('tbl_car', $condition, null);
                    if ($regCar){
                        $pass = true;
                        $condition = array(
                        ...
                        $join                       = 'tbl_offence';
                        $value                      = 'tbl_offence.offence_id = tbl_violation.offence_id';
                        $this->page_data['datas']    = $this->get('tbl_violation',$condition,true,$join,$value);
                    }else{
                        $this->page_data['message'] = array('class' => 'danger', 'header' => 'Error !', 'description' => 'Car is not registered.');
                    }
                }else{
                    $this->page_data['message'] = array('class' => 'danger', 'header' => 'Error !', 'description' => 'Phone number is not registered.');
                }
            }else{
                $this->page_data['message'] = array('class' => 'danger', 'header' => 'Error !', 'description' => $this->formValidationErrors());
            }
        }
        $this->page_data['pass']			= $pass;
        $this->load->view('auth/__frmOwner',$this->page_data);
    }
    ...
    public function notify(){
        $number         = 255625810480;
        $message        = "DRAINAGE CLOGGED AT UPANGA LID_111";
